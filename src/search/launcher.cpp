...
struct Launcher::Impl {
    ...
    winrt::Windows::UI::ViewManagement::UISettings appearance;ULONGLONG animationStart=0;bool highContrast=false;float textScale=1;COLORREF surface=RGB(14,18,25),inputSurface=RGB(26,33,43),textColor=RGB(232,242,245),secondary=RGB(145,163,176),selection=RGB(30,56,61),accent=RGB(121,224,199);
    Paint painter;HMONITOR display=nullptr;MonitorFollow follower;bool placing=false,opening=false;int focusAttempts=0;HHOOK outsideHook{};inline static Impl* hookOwner=nullptr;HFONT inputFont{},smallFont{},monoFont{};HFONT font{},symbolFont{};HBRUSH background=CreateSolidBrush(RGB(14,18,25)),inputBackground=CreateSolidBrush(RGB(26,33,43));
    ...
    ~Impl(){hide(false);{std::lock_guard lock(mutex);stop=true;}wake.notify_one();worker.join();localIndex.reset();DestroyWindow(hwnd);DeleteObject(inputFont);DeleteObject(smallFont);DeleteObject(monoFont);DeleteObject(font);DeleteObject(symbolFont);DeleteObject(background);DeleteObject(inputBackground);}
    ...
    void layout(){dpi=GetDpiForWindow(hwnd);HIGHCONTRASTW hc{sizeof hc};SystemParametersInfoW(SPI_GETHIGHCONTRAST,sizeof hc,&hc,0);highContrast=(hc.dwFlags&HCF_HIGHCONTRASTON)!=0;surface=highContrast?GetSysColor(COLOR_WINDOW):RGB(14,18,25);inputSurface=highContrast?surface:RGB(26,33,43);textColor=highContrast?GetSysColor(COLOR_WINDOWTEXT):RGB(232,242,245);secondary=highContrast?textColor:RGB(145,163,176);selection=highContrast?GetSysColor(COLOR_HIGHLIGHT):RGB(30,56,61);accent=highContrast?textColor:RGB(121,224,199);DeleteObject(background);background=CreateSolidBrush(surface);DeleteObject(inputBackground);inputBackground=CreateSolidBrush(inputSurface);
        ...
        DeleteObject(inputFont);DeleteObject(smallFont);DeleteObject(monoFont);
        inputFont=CreateFontW(-px(int(23*textScale)),0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Segoe UI Variable");
        ...
        monoFont=CreateFontW(-px(int(11*textScale)),0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Cascadia Code");
        int top=px(158),footer=px(102),rowHeight=px(int(55*textScale)),listSpace=std::max(1,int(r.bottom)-top-footer);
        MoveWindow(edit,px(76),px(72),r.right-px(150),px(int(33*textScale)),TRUE);
        MoveWindow(heading,px(30),px(132),r.right-px(60),px(int(20*textScale)),TRUE);
        int listHeight=listSpace>=rowHeight?(listSpace/rowHeight)*rowHeight:listSpace;
        MoveWindow(list,px(18),top,std::max(1,int(r.right)-px(36)),listHeight,TRUE);
        MoveWindow(status,px(29),r.bottom-px(91),r.right-px(58),px(22),TRUE);
        int buttonY=r.bottom-px(56),buttonHeight=px(40),available=r.right-px(44),unit=available/3;
        MoveWindow(openButton,px(22),buttonY,unit-px(8),buttonHeight,TRUE);
        MoveWindow(addButton,px(22)+unit,buttonY,unit-px(8),buttonHeight,TRUE);
        MoveWindow(actionButton,px(22)+unit*2,buttonY,unit-px(8),buttonHeight,TRUE);
        ...
        SendMessageW(edit,WM_SETFONT,reinterpret_cast<WPARAM>(inputFont),TRUE);SendMessageW(heading,WM_SETFONT,reinterpret_cast<WPARAM>(monoFont),TRUE);SendMessageW(status,WM_SETFONT,reinterpret_cast<WPARAM>(smallFont),TRUE);
        if(old)DeleteObject(old);SendMessageW(list,LB_SETITEMHEIGHT,0,rowHeight);updateActions();InvalidateRect(hwnd,nullptr,FALSE);
    }
    ...
    static LRESULT CALLBACK outsideClick(int code,WPARAM event,LPARAM data){auto p=hookOwner;if(code==HC_ACTION&&p&&(event==WM_LBUTTONDOWN||event==WM_RBUTTONDOWN||event==WM_MBUTTONDOWN)&&!p->actionMenu){auto point=reinterpret_cast<MSLLHOOKSTRUCT*>(data)->pt;auto hit=WindowFromPoint(point);if(hit!=p->hwnd&&!IsChild(p->hwnd,hit))PostMessageW(p->hwnd,WM_APP+82,0,0);}return CallNextHookEx(nullptr,code,event,data);}

    bool focusInput(){auto foreground=GetForegroundWindow();auto sourceThread=GetWindowThreadProcessId(foreground,nullptr),searchThread=GetCurrentThreadId();bool joined=sourceThread&&sourceThread!=searchThread&&AttachThreadInput(searchThread,sourceThread,TRUE);BringWindowToTop(hwnd);SetForegroundWindow(hwnd);SetActiveWindow(hwnd);SetFocus(edit);if(joined)AttachThreadInput(searchThread,sourceThread,FALSE);return GetForegroundWindow()==hwnd&&GetFocus()==edit;}
    void show(){if(IsWindowVisible(hwnd)){hide(true);return;}previous=GetForegroundWindow();POINT point{};GetCursorPos(&point);placeOn(MonitorFromPoint(point,MONITOR_DEFAULTTONEAREST));follower.reset();if(GetSystemMetrics(SM_CMONITORS)>1)SetCoalescableTimer(hwnd,92,120,nullptr,30);SetWindowTextW(edit,L"");BOOL animate=TRUE;SystemParametersInfoW(SPI_GETCLIENTAREAANIMATION,0,&animate,0);bool motion;{std::lock_guard lock(mutex);motion=animate&&!options.reduceMotion&&!highContrast;}SetLayeredWindowAttributes(hwnd,0,motion?80:255,LWA_ALPHA);opening=true;ShowWindow(hwnd,SW_SHOWNORMAL);focusAttempts=0;if(focusInput())opening=false;else SetTimer(hwnd,93,45,nullptr);if(!review){hookOwner=this;outsideHook=SetWindowsHookExW(WH_MOUSE_LL,outsideClick,GetModuleHandleW(nullptr),0);if(!outsideHook)hookOwner=nullptr;}SendMessageW(edit,EM_SETSEL,0,-1);query();if(motion){animationStart=GetTickCount64();SetTimer(hwnd,91,16,nullptr);}}
    void hide(bool restore){opening=false;KillTimer(hwnd,92);KillTimer(hwnd,93);if(outsideHook){UnhookWindowsHookEx(outsideHook);outsideHook=nullptr;}if(hookOwner==this)hookOwner=nullptr;follower.reset();KillTimer(hwnd,91);++generation;ShowWindow(hwnd,SW_HIDE);if(restore&&IsWindow(previous))SetForegroundWindow(previous);}
    ...
    void actions(std::optional<POINT> at={}){int index=int(SendMessageW(list,LB_GETCURSEL,0,0));if(index<0||index>=int(shown.size()))return;auto menu=CreatePopupMenu();AppendMenuW(menu,MF_STRING,1,L"Open    Enter");auto kind=shown[index].kind;if(kind==Kind::File||kind==Kind::Folder){AppendMenuW(menu,MF_SEPARATOR,0,nullptr);AppendMenuW(menu,MF_STRING,2,L"Open file location in Explorer");AppendMenuW(menu,MF_STRING,4,L"Copy full path");AppendMenuW(menu,MF_SEPARATOR,0,nullptr);AppendMenuW(menu,MF_STRING,5,L"Add to Floatlet    Shift+Enter");AppendMenuW(menu,MF_STRING,6,L"Collect and keep searching    Ctrl+Enter");if(kind==Kind::File)AppendMenuW(menu,MF_STRING,3,L"Copy file");}RECT r;GetWindowRect(hwnd,&r);POINT point=at.value_or(POINT{r.right-px(240),r.bottom-px(52)});actionMenu=true;int action=TrackPopupMenu(menu,TPM_RETURNCMD|TPM_NONOTIFY,point.x,point.y,0,hwnd,nullptr);actionMenu=false;DestroyMenu(menu);if(action)act(action);}
    ...
    static LRESULT CALLBACK proc(HWND h,UINT m,WPARAM w,LPARAM l){auto p=reinterpret_cast<Impl*>(GetWindowLongPtrW(h,GWLP_USERDATA));if(m==WM_NCCREATE){p=static_cast<Impl*>(reinterpret_cast<CREATESTRUCTW*>(l)->lpCreateParams);SetWindowLongPtrW(h,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(p));}if(!p)return DefWindowProcW(h,m,w,l);
        switch(m){case WM_TIMER:if(w==92){p->follow();return 0;}if(w==93){if(p->focusInput()){p->opening=false;KillTimer(h,93);}else if(++p->focusAttempts>=6){p->opening=false;KillTimer(h,93);SetWindowTextW(p->status,L"Click the search field to type");}return 0;}if(w==91){float t=std::min(1.f,float(GetTickCount64()-p->animationStart)/160.f);float eased=1-(1-t)*(1-t)*(1-t);SetLayeredWindowAttributes(h,0,BYTE(80+175*eased),LWA_ALPHA);if(t>=1)KillTimer(h,91);}return 0;case WM_COMMAND:if(LOWORD(w)==403)p->act(1);if(LOWORD(w)==404)p->act(5);if(LOWORD(w)==405)p->actions();if(LOWORD(w)==Edit&&HIWORD(w)==EN_CHANGE)p->query();if(LOWORD(w)==List&&HIWORD(w)==LBN_SELCHANGE){p->navigated=true;p->updateActions();}if(LOWORD(w)==List&&HIWORD(w)==LBN_DBLCLK)p->open();return 0;case WM_APP+81:if(IsWindowVisible(h))p->query();return 0;case WM_APP+82:if(IsWindowVisible(h))p->hide(false);return 0;case Ready:p->accept();return 0;case WM_ACTIVATE:if(LOWORD(w)!=WA_INACTIVE&&IsWindowVisible(h)&&GetFocus()!=p->edit)SetFocus(p->edit);else if(!p->review&&!p->actionMenu&&!p->opening&&LOWORD(w)==WA_INACTIVE)p->hide(false);return 0;case WM_CLOSE:p->hide(true);return 0;case WM_SETTINGCHANGE:p->layout();InvalidateRect(h,nullptr,TRUE);return 0;case WM_DISPLAYCHANGE:{KillTimer(h,92);p->follower.reset();if(IsWindowVisible(h)&&GetSystemMetrics(SM_CMONITORS)>1)SetCoalescableTimer(h,92,120,nullptr,30);POINT point{};GetCursorPos(&point);p->placeOn(MonitorFromPoint(point,MONITOR_DEFAULTTONEAREST));return 0;}case WM_DPICHANGED:{if(p->placing)return 0;auto r=reinterpret_cast<RECT*>(l);SetWindowPos(h,nullptr,r->left,r->top,r->right-r->left,r->bottom-r->top,SWP_NOZORDER|SWP_NOACTIVATE);p->layout();return 0;}
        case WM_ERASEBKGND:return 1;
        case WM_PAINT:{PAINTSTRUCT ps;BeginPaint(h,&ps);RECT bounds;GetClientRect(h,&bounds);try{auto& paint=p->painter;paint.begin(ps.hdc,bounds);paint.clear(p->surface);float scale=p->dpi/96.f,width=float(bounds.right),height=float(bounds.bottom);if(!p->highContrast){paint.outline(1,1,width-2,height-2,16*scale,RGB(53,68,77));paint.round(24*scale,24*scale,7*scale,7*scale,3.5f*scale,p->accent);paint.round(22*scale,58*scale,width-44*scale,58*scale,14*scale,p->inputSurface);paint.outline(22*scale,58*scale,width-44*scale,58*scale,14*scale,RGB(51,73,81));paint.text(L"⌕",40*scale,69*scale,25*scale,34*scale,24*scale,p->accent);paint.round(22*scale,height-103*scale,width-44*scale,1*scale,0,p->inputSurface);}paint.text(L"FLOATLET  /  SEARCH",42*scale,19*scale,260*scale,20*scale,11*scale,p->accent,false,true);paint.text(L"ESC  CLOSE",width-115*scale,19*scale,95*scale,20*scale,10*scale,p->secondary,false,true);paint.end();}catch(...){}EndPaint(h,&ps);return 0;}
        case WM_CTLCOLOREDIT:{auto dc=reinterpret_cast<HDC>(w);SetTextColor(dc,p->textColor);SetBkColor(dc,p->inputSurface);return reinterpret_cast<LRESULT>(p->inputBackground);}
        case WM_CTLCOLORSTATIC:case WM_CTLCOLORLISTBOX:SetTextColor(reinterpret_cast<HDC>(w),m==WM_CTLCOLORSTATIC&&reinterpret_cast<HWND>(l)==p->heading?p->accent:p->secondary);SetBkColor(reinterpret_cast<HDC>(w),p->surface);return reinterpret_cast<LRESULT>(p->background);
        case WM_CONTEXTMENU:if(reinterpret_cast<HWND>(w)==p->list){POINT point{GET_X_LPARAM(l),GET_Y_LPARAM(l)};if(point.x==-1&&point.y==-1){int selected=int(SendMessageW(p->list,LB_GETCURSEL,0,0));if(selected<0)return 0;RECT item{};if(SendMessageW(p->list,LB_GETITEMRECT,selected,reinterpret_cast<LPARAM>(&item))==LB_ERR)return 0;point={item.left+p->px(60),item.bottom};ClientToScreen(p->list,&point);}else{POINT local=point;ScreenToClient(p->list,&local);auto hit=SendMessageW(p->list,LB_ITEMFROMPOINT,0,MAKELPARAM(local.x,local.y));if(HIWORD(hit)||LOWORD(hit)>=p->shown.size())return 0;SendMessageW(p->list,LB_SETCURSEL,LOWORD(hit),0);p->navigated=true;p->updateActions();}int selected=int(SendMessageW(p->list,LB_GETCURSEL,0,0));if(selected>=0&&selected<int(p->shown.size())&&(p->shown[selected].kind==Kind::File||p->shown[selected].kind==Kind::Folder))SetWindowTextW(p->status,p->shown[selected].target.c_str());p->actions(point);return 0;}return DefWindowProcW(h,m,w,l);
        case WM_DRAWITEM:{auto d=reinterpret_cast<DRAWITEMSTRUCT*>(l);auto rect=d->rcItem;float scale=p->dpi/96.f,wid=float(rect.right-rect.left),height=float(rect.bottom-rect.top);auto& paint=p->painter;try{paint.begin(d->hDC,rect);paint.clear(p->surface);
            if(d->CtlType==ODT_BUTTON){bool disabled=(d->itemState&ODS_DISABLED)!=0,primary=d->CtlID==403;paint.round(0,0,wid,height,11*scale,p->highContrast?GetSysColor(COLOR_BTNFACE):primary?p->accent:p->inputSurface);if(!p->highContrast&&!primary)paint.outline(.5f,.5f,wid-1,height-1,11*scale,RGB(46,63,72));wchar_t label[80]{};GetWindowTextW(d->hwndItem,label,80);paint.text(label,16*scale,(height-19*scale*p->textScale)/2,wid-24*scale,height,12*scale*p->textScale,disabled?p->secondary:p->highContrast?GetSysColor(COLOR_BTNTEXT):primary?RGB(12,35,35):p->textColor,true);paint.end();if(d->itemState&ODS_FOCUS){InflateRect(&rect,-4,-4);DrawFocusRect(d->hDC,&rect);}return TRUE;}
            ...
            if(selected){paint.round(0,2*scale,wid,height-4*scale,11*scale,p->selection);if(!p->highContrast)paint.round(1*scale,12*scale,3*scale,height-24*scale,1.5f*scale,p->accent);}
            paint.round(13*scale,9*scale,37*scale,37*scale,10*scale,p->highContrast?GetSysColor(COLOR_BTNFACE):p->inputSurface);
            paint.text(item.title,63*scale,6*scale,wid-100*scale,26*scale*p->textScale,14*scale*p->textScale,fg,true);
            paint.text(item.detail,63*scale,7*scale+22*scale*p->textScale,wid-88*scale,21*scale*p->textScale,10.5f*scale*p->textScale,muted);
            if(selected)paint.text(L"\u21b5",wid-27*scale,(height-20*scale)/2,20*scale,22*scale,14*scale,p->accent);
            if(!item.icon){const wchar_t* symbol=item.kind==Kind::Command?(item.title.find(L"Calendar")!=std::wstring::npos?L"\u25a6":item.title.find(L"Music")!=std::wstring::npos?L"\u266b":item.title.find(L"Preferences")!=std::wstring::npos?L"\u2699":item.title.find(L"Alarm")!=std::wstring::npos?L"\u25f7":L"\u229e"):L"\u25a1";paint.text(symbol,20*scale,(height-27*scale)/2,28*scale,28*scale,22*scale,selected?p->accent:fg);}
            paint.end();if(item.icon)DrawIconEx(d->hDC,rect.left+p->px(17),rect.top+(int(height)-p->px(28))/2,static_cast<HICON>(item.icon.get()),p->px(28),p->px(28),0,nullptr,DI_NORMAL);return TRUE;
        }catch(...){return TRUE;}}}
