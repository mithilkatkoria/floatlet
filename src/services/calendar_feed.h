...
namespace delight {
struct AgendaSnapshot {ical::Agenda agenda;std::wstring status=L"Connect a calendar in Preferences";int year=0;unsigned month=0,day=0;bool connected=false;};
class CalendarFeed {
    ...
    std::wstring url;unsigned generation=0;std::chrono::year_month_day selected;AgendaSnapshot snapshot;std::thread worker;
    ...
    void run(){ical::Calendar cache;ULONGLONG fetched=0;unsigned cachedGeneration=~0u;
        for(;;){std::wstring request;std::chrono::year_month_day date;unsigned version;bool reload;
            {std::unique_lock lock(mutex);cv.wait(lock,[&]{return done||pending;});if(done)return;pending=false;request=url;date=selected;version=generation;reload=force;force=false;}
            ...
            if(!request.empty())try{if(reload||!fetched||GetTickCount64()-fetched>15*60*1000){cache=ical::parse(download(request));fetched=GetTickCount64();}next.agenda=ical::day(cache,date);next.status=next.agenda.unsupported?L"Some calendar rules are not supported":L"Read-only calendar. Refresh for changes.";}
            catch(...){next.status=L"Could not refresh calendar. Check its link.";if(fetched&&cachedGeneration==version)try{next.agenda=ical::day(cache,date);next.status=L"Offline. Showing the last loaded calendar.";}catch(...) {}}
