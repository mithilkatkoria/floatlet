...
void Renderer::draw(float w,float h,float dpi,const std::vector<Line>& lines,bool animate,bool highContrast,const std::vector<Mark>& marks,std::shared_ptr<const Artwork> artwork) {
    ...
    for(auto& mark:marks){
        ...
        case Glyph::Timer:{brush->SetOpacity(.16f);ellipse(.5f,.5f,.42f);brush->SetOpacity(1);if(!highContrast)brush->SetColor(D2D1::ColorF(1.f,.64f,.28f));float fraction=std::clamp(mark.value,0.f,1.f);int segments=std::max(1,int(96*fraction));for(int i=0;i<segments;++i){float a=-1.5707963f+6.2831853f*fraction*i/segments,b=-1.5707963f+6.2831853f*fraction*(i+1)/segments;dc->DrawLine({x+s*(.5f+.42f*cosf(a)),y+s*(.5f+.42f*sinf(a))},{x+s*(.5f+.42f*cosf(b)),y+s*(.5f+.42f*sinf(b))},brush.get(),std::max(2.f,s*.06f));}break;}
