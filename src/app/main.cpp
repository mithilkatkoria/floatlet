    ...
    void render(bool animate=false){
        ...
        if(model.state==State::Peek&&!notice.empty()){marks.push_back({noticeGlyph,18,19,28,true,false,28,noticeGlyph==Glyph::AlarmBell&&!settings.reduceMotion?sinf(GetTickCount64()*.035f)*9.f:0.f});add(notice,61,14,15,false,220);add(noticeDetail,61,37,10,true,220);}
            ...
            if(model.state==State::Music){
                ...
                add(L"<  Settings",20,49,12,true,110);add(L"Search",20,73,20);icon(Glyph::Toggle,294,75,34,settings.search.enabled);
                add(L"SEARCH ENGINE",20,112,10,true);icon(Glyph::Chip,16,133,155,settings.search.provider==0);icon(Glyph::Chip,181,133,155,settings.search.provider==1);add(L"Everything",16,140,12,settings.search.provider!=0,155,true);add(L"Floatlet index",181,140,12,settings.search.provider!=1,155,true);
                add(settings.search.provider==0?L"Uses the Everything or Windows Search index":L"Private filename index, built into Floatlet",20,171,10,true,316);
                add(L"BUILT-IN INDEX SCOPE",20,205,10,true);icon(Glyph::Chip,16,227,155,settings.search.provider==1&&settings.search.scope==0,settings.search.provider==0);icon(Glyph::Chip,181,227,155,settings.search.provider==1&&settings.search.scope==1,settings.search.provider==0);add(L"Personal folders",16,234,11,settings.search.provider!=1||settings.search.scope!=0,155,true);add(L"Local drives",181,234,11,settings.search.provider!=1||settings.search.scope!=1,155,true);
                add(L"INCLUDE",20,275,10,true);
                ...
                for(int i=0;i<4;++i){float x=16+(i%2)*165.f,y=296+(i/2)*38.f;marks.push_back({Glyph::Card,x,y,155,false,false,32});add(labels[i],x+10,y+9,10.5f,false,115);icon(Glyph::Toggle,x+119,y+7,28,values[i]);}
                add(L"SHORTCUT",20,389,10,true);marks.push_back({Glyph::Card,16,410,320,false,false,37});add(search::shortcutLabel(settings.search),28,420,12,false,292);
                icon(Glyph::Chip,16,457,155,searchRecorder.recording());icon(Glyph::Chip,181,457,155);add(searchRecorder.recording()?L"Press shortcut...":L"Record shortcut",16,464,11,false,155,true);add(L"Right Alt + Space",181,464,11,false,155,true);
