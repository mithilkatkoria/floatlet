...
Website playground update: eight modes including clocks, calendars, calls and settings. Tested at 360px and 1280px viewports, with no horizontal page overflow. Browser checks covered sample next-track playback, running timer completion, alert persistence after Escape, explicit dismissal, slider updates, tray addition/removal, month navigation, Show calls and reset. No browser console errors observed. Typecheck clean, six automated tests passed, static build and 184 links/assets passed. All demo data is fictional; no account, native audio, microphone or filesystem access.
